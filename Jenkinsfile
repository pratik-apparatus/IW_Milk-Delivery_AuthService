pipeline {
    ...
    environment {
        CONTAINER_PORT = '3001'
        IMAGE_NAME = 'milk-delivery-auth'
        NETWORK_NAME = 'milk-delivery-services'
        SERVICE_ALIAS = 'auth.milk-delivery'
    }
    ...
    stages {
            ...
            steps {
                script {
                    ...
                    if (prodBranches.contains(env.BRANCH_NAME)) {
                        ...
                        env.ENV_CREDENTIAL_ID = 'milk_delivery_auth_env'
                    } else {
                        env.NODE_ENV = 'development'
                        env.ENV_CREDENTIAL_ID = 'milk_delivery_auth_env_dev'
                    }
        ...
        stage('Set Port and Container Name') {
            steps {
                script {
                    env.HOST_PORT = '4011'
                    env.CONTAINER_NAME = "milk-delivery-auth-${env.BRANCH_NAME}"
                }
            }
        }
                ...
                withCredentials([file(credentialsId: "${ENV_CREDENTIAL_ID}", variable: 'ENV_FILE')]) {
                    sh '''
                        ...
                        docker run -d \
                            ...
                            -e BACKEND_MS_HOST="backend.milk-delivery" \
                            -e BACKEND_MS_PORT="4012" \
